...
public class ResourceServerConfig extends ResourceServerConfigurerAdapter {
    ...
    public void configure(HttpSecurity http) throws Exception {
        http.requestMatcher(request -> {
            ...
            //这里并没有做jwt的验证签名操作---------------------------------------------------------------------
            System.err.println(StringUtils.hasText(token) && token.startsWith("Bearer ") ? true : false);
            System.out.println(request.getParameter("access_token") != null);
            return true;
//            return StringUtils.hasText(token) && token.startsWith("Bearer ") ? true : false || request.getParameter("access_token") != null;
                ...
                //用以判断实际发送的请求是否安全
                .antMatchers(HttpMethod.OPTIONS).permitAll()//这里通过接口的访问方式开放,实际并不会这么做
